...
pipeline {
    ...
    stage('SonarQube Inspection') {
        steps {
            withSonarQubeEnv('SonarQube') { 
                ...
                -Dsonar.host.url=http://172.31.61.240:9000 \
    ...
    stage("Nexus Artifact Uploader"){
        steps{
           nexusArtifactUploader(
              nexusVersion: 'nexus3',
              protocol: 'http',
              nexusUrl: '172.31.51.19:8081',
              groupId: 'webapp',
              version: "${env.BUILD_ID}-${env.BUILD_TIMESTAMP}",
              repository: 'Java-webapp-project-snapshot',  //"${NEXUS_REPOSITORY}",
  ...
  post {
    always {
        ...
        slackSend channel: '#gabinator_complete_cicd_pipeline_alerts', //update and provide your channel name
